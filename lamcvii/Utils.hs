...
sortOf :: Objects -> Context -> Term -> Term
sortOf sig ctx t = case t of
  Box -> error "sort of box"
  Star -> Box
  Var n -> hypType (fromMaybe (error "!! in sortOf" ) (nth n ctx))
  App fun xs -> sortOf sig ctx fun
  Lam _ n ta b -> sortOf sig (Hypothesis n ta Nothing : ctx) b
  Pi _ n ta tb -> sortOf sig (Hypothesis n ta Nothing : ctx) tb
  Let n ta a b -> sortOf sig (Hypothesis n ta (Just a) : ctx) b
  Case dat -> sortOf sig ctx (motive dat)
  Const _ ref -> typeofRef sig ref
